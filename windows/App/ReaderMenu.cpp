...
ReaderMenu makeReaderMenu(MenuActions actions) {
    ...
    item = add(file, command(L"Open URL…", L"action.openURL", actions.openURL, L"\uE774"));
    shortcut(item, VirtualKey::L, control, L"Ctrl+L");
    ...
    item = add(file, command(L"Export PDF…", L"action.exportPDF", actions.exportPDF, L"\uE749"), true);
    shortcut(item, VirtualKey::P, control, L"Ctrl+P");
    ...
    add(file, command(L"Cancel Current Operation", L"action.cancelLoading", actions.cancelLoading));
    file.Items().Append(MenuFlyoutSeparator());
